...
해당 클래스는 추상메서드를 포함하고 있으므로 하위에서 상속받게되면 반드시
추상메서드를 오버라이딩 해야한다.  */
...
DTO(Data Transfer Object) : 데이터를 저장하고 전송하기 위한 용도의
 객체를 말한다. VO(Value Object)로 표현하기도한다. 
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {	
	...
	해야한다. 이는 필수사항이며 하지않으면 에러가 발생하게된다.  
	*/
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i=0 ; i<numOfPerInfo ; i++) {
			...
			선언된 멤버이므로 직접 접근이 불가능하다. 따라서 아래와같이
			getter를 통해 값을 반환받아 사용해야한다. */
